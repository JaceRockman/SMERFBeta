...
(defn realm-details [conn subrealm-data]
  [:> rn/ScrollView {:style {:flex :1}}
   (components/default-realm-markdown (:realm/entity-details subrealm-data))])
...
(defn realm-home [conn]
  (let [active-campaign-data (campaign-data/get-active-campaign conn)
    ...
    (cond
      ...
      active-campaign-data (realm-select (campaign-data/get-active-campaign-realms conn))
      :else (realm-select (realm-data/get-all-realms conn)))))
