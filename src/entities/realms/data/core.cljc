...
(defn get-active-realm-id
  [conn]
  (let [{:keys [page query-params]} (navigation/get-nav-state conn)]
    (when (and (= :realms page)
               (< 0 (count query-params)))
      (int (first query-params)))))
...
(defn set-active-realm
  [conn realm-id]
  (navigation/subnavigate! conn {:page :realms :query-params [realm-id]}))
...
(defn get-active-subrealm-id
  ...
  (let [{:keys [page query-params]} (navigation/get-nav-state conn)]
    (when (and (= :realms page)
               (= 3 (count query-params)))
      (int (last query-params)))))
...
(defn set-active-subrealm
  [conn subrealm-id]
  (navigation/subnavigate! conn {:page :realms :query-params [subrealm-id]}))
